...
describe("isPublicAuthPath", () => {
  it.each(["/", "/evaluacion-inicial", "/evaluacion-inicial/nueva", "/evaluacion-inicial/local-id", "/auth/login", "/auth/callback", "/auth/confirm", "/auth/activate", "/auth/invitation-session", "/auth/forgot-password", "/auth/reset-password"])("allows %s without a session", (path) => {
