...
export async function loadWorkspaceContext(organizationId: string) {
  ...
  const [organizations, { data: profile }, permissions] = await Promise.all([
    listOrganizations(),
  ...
  return { context, organizations, isDemo: settings?.is_demo === true || tenant.organization.slug === "empresa-demo-colombia" };
}
