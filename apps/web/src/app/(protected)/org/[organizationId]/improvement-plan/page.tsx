...
export default async function ImprovementPlanPage({ params, searchParams }: { params: Promise<{ organizationId: string }>; searchParams: Promise<{ status?: string; q?: string; state?: string; priority?: string; origin?: string; actionState?: string; focus?: string; page?: string }> }) {
  const { organizationId } = await params;
  await requireTenant(organizationId);
  const { status, q = "", state = "all", priority = "all", origin = "all", actionState = "all", focus, page = "1" } = await searchParams;
  ...
  const pageHref = (nextPage: number) => `/org/${organizationId}/improvement-plan?${new URLSearchParams({ ...(q ? { q } : {}), ...(state !== "all" ? { state } : {}), ...(priority !== "all" ? { priority } : {}), ...(origin !== "all" ? { origin } : {}), ...(actionState !== "all" ? { actionState } : {}), page: String(nextPage) }).toString()}`;

  return <div className="grid gap-7">
    <PageHeader eyebrow="Cierre de oportunidades de mejora" title="Plan de mejoramiento" description="Gestiona oportunidades de mejora, acciones, evidencias y validación sin duplicar el resultado de los recálculos." />
    ...
    <section className="grid gap-5" aria-label="Oportunidades de mejora y acciones">{gaps.length ? gaps.map((gap) => {
      ...
      return <Card key={gap.id}><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-semibold">{gap.title}</h2><Badge>{originLabels[gap.origin_type] ?? "Otro origen"}</Badge><StatusBadge status={gap.priority === "critical" ? "danger" : gap.priority === "high" ? "warning" : "inactive"}>{priorityLabels[gap.priority] ?? gap.priority}</StatusBadge><StatusBadge status={gap.status === "resolved" ? "active" : "pending"}>{gap.status}</StatusBadge></div>{gap.description ? <p className="mt-2 text-sm text-[var(--muted)]">{gap.description}</p> : null}<p className="mt-2 text-xs text-[var(--muted)]">Origen: {originLabels[gap.origin_type] ?? "Otro"} · Detectada: {new Date(gap.created_at).toLocaleDateString("es-CO")}</p></div><div className="flex flex-wrap gap-2">{mayManage && gap.status !== "resolved" ? <FormDrawer triggerLabel="Nueva acción" title="Nueva acción de mejora" description="Define el trabajo, responsable, prioridad y fecha objetivo para cerrar esta oportunidad de mejora."><ImprovementActionCreateForm organizationId={organizationId} gapId={gap.id} defaultPriority={gap.priority} members={(membersResult.data ?? []).map((member) => ({ id: member.user_id, label: profiles.get(member.user_id) ?? "Persona sin nombre" }))} /></FormDrawer> : null}{mayValidate && gap.status !== "resolved" && hasVerifiedAction ? <form action={closeImprovementGap}><input type="hidden" name="organizationId" value={organizationId} /><input type="hidden" name="gap_id" value={gap.id} /><Button type="submit" size="sm" variant="secondary">Cerrar oportunidad validada</Button></form> : null}</div></div></CardHeader><CardContent className="grid gap-5">
        ...
        </article>) : <EmptyState title="Sin acciones" description="Crea una acción concreta para gestionar esta oportunidad de mejora sin afectar las acciones generadas automáticamente." />}</div>
      </CardContent></Card>;
    }) : <EmptyState title="Sin oportunidades detectadas" description="Los resultados no cumplidos de una evaluación crearán una oportunidad de mejora y su acción inicial sin duplicarse en recálculos posteriores." />}</section>
  </div>;
}
