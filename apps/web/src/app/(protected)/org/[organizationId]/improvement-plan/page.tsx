...
export default async function ImprovementPlanPage({ params, searchParams }: { params: Promise<{ organizationId: string }>; searchParams: Promise<{ status?: string; q?: string; state?: string; priority?: string; origin?: string; page?: string }> }) {
  ...
  return <div className="grid gap-7">
    ...
    <section className="grid gap-5" aria-label="Brechas y acciones">{gaps.length ? gaps.map((gap) => {
      ...
      return <Card key={gap.id}><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex flex-wrap items-center gap-2"><h2 className="font-semibold">{gap.title}</h2><Badge>{gap.origin_type}</Badge><StatusBadge status={gap.priority === "critical" ? "danger" : gap.priority === "high" ? "warning" : "inactive"}>{gap.priority}</StatusBadge><StatusBadge status={gap.status === "resolved" ? "active" : "pending"}>{gap.status}</StatusBadge></div>{gap.description ? <p className="mt-2 text-sm text-[var(--muted)]">{gap.description}</p> : null}<p className="mt-2 text-xs text-[var(--muted)]">Origen: {gap.origin_type} · Detectada: {new Date(gap.created_at).toLocaleDateString("es-CO")}</p></div>{mayValidate && gap.status !== "resolved" && hasVerifiedAction ? <form action={closeImprovementGap}><input type="hidden" name="organizationId" value={organizationId} /><input type="hidden" name="gap_id" value={gap.id} /><Button type="submit" size="sm" variant="secondary">Cerrar brecha validada</Button></form> : null}</div></CardHeader><CardContent className="grid gap-5">
        {mayManage && gap.status !== "resolved" ? <ImprovementActionCreateForm organizationId={organizationId} gapId={gap.id} defaultPriority={gap.priority} members={(membersResult.data ?? []).map((member) => ({ id: member.user_id, label: profiles.get(member.user_id) ?? member.user_id }))} /> : null}
        <div className="grid gap-4">{gapActions.length ? gapActions.map((action) => <article key={action.id} className="grid gap-4 rounded-lg border border-[var(--border)] p-4"><div><div className="flex flex-wrap items-center gap-2"><strong>{action.title}</strong><StatusBadge status={action.status === "verified" ? "active" : action.status === "evidence_submitted" ? "pending" : action.status === "cancelled" ? "danger" : "inactive"}>{action.status}</StatusBadge><StatusBadge status={action.priority === "critical" ? "danger" : action.priority === "high" ? "warning" : "inactive"}>{action.priority}</StatusBadge></div>{action.description ? <p className="mt-2 text-sm text-[var(--muted)]">{action.description}</p> : null}<p className="mt-2 text-xs text-[var(--muted)]">{formatDate(action.target_date)} · {action.responsible_user_id ? `Responsable: ${profiles.get(action.responsible_user_id) ?? "Usuario"}` : "Sin responsable"} · {action.evidence_document_version_id ? "Evidencia vinculada" : "Sin evidencia"}</p>{action.validation_note ? <p className="mt-2 text-xs text-[var(--muted)]">Validación: {action.validation_note}</p> : null}</div>
          {mayManage && actionStatusOptions(action.status).length ? <form action={updateImprovementAction} className="grid gap-3 rounded-lg bg-[var(--muted-surface)] p-3 md:grid-cols-2"><input type="hidden" name="organizationId" value={organizationId} /><input type="hidden" name="action_id" value={action.id} /><input type="hidden" name="evidence_document_version_id" value={action.evidence_document_version_id ?? ""} /><input type="hidden" name="validation_note" value={action.validation_note ?? ""} /><label className="grid gap-1 text-sm font-medium md:col-span-2">Título<Input name="title" required defaultValue={action.title} /></label><label className="grid gap-1 text-sm font-medium md:col-span-2">Descripción<Textarea name="description" defaultValue={action.description ?? ""} /></label><label className="grid gap-1 text-sm font-medium">Estado<Select name="status" defaultValue={action.status}>{actionStatusOptions(action.status).map((item) => <option key={item} value={item}>{item}</option>)}</Select></label><label className="grid gap-1 text-sm font-medium">Prioridad<Select name="priority" defaultValue={action.priority}>{["critical", "high", "medium", "low"].map((item) => <option key={item} value={item}>{item}</option>)}</Select></label><label className="grid gap-1 text-sm font-medium">Responsable<Select name="responsible_user_id" defaultValue={action.responsible_user_id ?? ""}><option value="">Sin asignar</option>{membersResult.data?.map((member) => <option key={member.id} value={member.user_id}>{profiles.get(member.user_id) ?? member.user_id}</option>)}</Select></label><label className="grid gap-1 text-sm font-medium">Fecha objetivo<Input name="target_date" type="date" defaultValue={action.target_date ?? ""} /></label><div><Button type="submit" size="sm" variant="secondary">Guardar acción</Button></div></form> : null}
