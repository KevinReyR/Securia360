...
export default async function PlanningPage({ params, searchParams }: {
  ...
  searchParams: Promise<{ q?: string; status?: string; taskStatus?: string; view?: string; focus?: string }>;
}) {
  const { organizationId } = await params;
  const tenant = await requireTenant(organizationId);
  const queryParameters = await searchParams;
  const { q = "", status: operationStatus, taskStatus, view = "board", focus } = queryParameters;
  ...
  if (selectedTaskStatus === "open") tasksQuery = tasksQuery.in("status", ["todo", "in_progress", "blocked"]);
  else if (selectedTaskStatus !== "all") tasksQuery = tasksQuery.eq("status", selectedTaskStatus);
  ...
  const taskSummary = [
    ["Tareas abiertas", tasks.filter((task) => !["completed", "cancelled"].includes(task.status)).length],
    ["Vencidas", tasks.filter((task) => task.due_at && new Date(task.due_at).getTime() < now && !["completed", "cancelled"].includes(task.status)).length],
    ["Sin responsable", tasks.filter((task) => !task.assigned_to && !["completed", "cancelled"].includes(task.status)).length],
  ...
  return <div className="grid gap-7">
    ...
    <SgsstFlowNav organizationId={organizationId} current="planning" />
    <StatusBanner status={bannerStatus} />
    <section aria-label="Resumen de trabajo" className="grid gap-px overflow-hidden rounded-[14px] border border-[var(--border)] bg-[var(--border)] sm:grid-cols-2 xl:grid-cols-4">{taskSummary.map(([label, value]) => <div key={label} className="bg-[var(--surface)] p-4"><p className="text-xs font-medium text-[var(--muted)]">{label}</p><p className="mt-1 text-2xl font-semibold tracking-[-0.04em]">{value}</p></div>)}</section>
    ...
    <Card><CardContent className="pt-5"><form className="grid gap-3 md:grid-cols-[minmax(0,1fr)_11rem_10rem_auto]"><Input name="q" defaultValue={q} placeholder="Buscar tareas" aria-label="Buscar tareas" /><Select name="taskStatus" defaultValue={selectedTaskStatus} aria-label="Estado"><option value="all">Todos los estados</option><option value="open">Abiertas</option>{taskStatusOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</Select><Select name="view" defaultValue={view} aria-label="Vista"><option value="board">Tablero</option><option value="calendar">Agenda</option></Select><Button variant="secondary">Aplicar filtros</Button></form></CardContent></Card>
