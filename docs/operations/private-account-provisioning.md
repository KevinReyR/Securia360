...
4. Si el correo aún no corresponde a una cuenta, Supabase envía una invitación de un solo uso. El enlace valida el token en `/auth/confirm` y abre `/auth/activate` para crear la contraseña y registrar nombres, apellidos y teléfono antes de continuar al onboarding empresarial. Si la cuenta ya existe, conserva su contraseña y perfil y acepta la nueva membresía al iniciar sesión.
...
- Copiar la plantilla versionada `supabase/templates/invite.html` en **Authentication → Email Templates → Invite user** del proyecto alojado. Debe conservar `TokenHash`, `type=invite`, `SiteURL` y `RedirectTo`; usar la plantilla predeterminada vuelve a introducir el flujo implícito que el servidor no puede procesar de forma segura.
