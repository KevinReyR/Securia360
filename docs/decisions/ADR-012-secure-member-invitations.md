...
El registro público está deshabilitado. Las cuentas nuevas se crean mediante invitación. Cuando existe una plantilla personalizada, esta usa `TokenHash` y `type=invite`; `/auth/confirm` muestra una confirmación humana y solo entonces verifica el token en el servidor, establece la sesión en cookies y activa las membresías pendientes. Cuando el proveedor alojado no permite personalizar la plantilla, `/auth/activate` recibe la sesión implícita emitida por `ConfirmationURL`, la persiste en cookies SSR y activa la membresía. Después, `/auth/activate` exige contraseña, nombres, primer apellido y teléfono antes de dirigir al administrador al onboarding. La función usa `APP_URL` como configuración server-side para construir únicamente redirecciones permitidas.
